...
WHERE (blogs.name LIKE CONCAT('%', ?, '%') OR blogs.short_description LIKE CONCAT('%', ?, '%'))";

$params = [$keywords, $keywords];
if (!empty($category)) {
    $sql .= " AND blog_category.blog_category_id = ?";
    $params[] = $category;
}

$sql .= " GROUP BY blogs.id";
                    ...
                    <?php foreach ($blogs as $blog): ?>
                        <article class="blog_item">
                            ...
                            <div class="blog_details">
                                <a class="d-inline-block" href="/single-blog.php?slug=<?= $blog['slug'] ?>">
                                    <h2><?= $blog['name'] ?></h2>
                                </a>
